...
if (!executablePath) throw new Error('A Chromium browser is required for the authentication smoke test.');
...
async function waitForSecurityLink(inboxId, subject, timeoutMs = 90_000) {
  ...
  while (Date.now() < deadline) {
    ...
    const message = listing.messages?.find((candidate) => candidate.subject === subject);
    if (message?.thread_id) {
      const thread = await agentMail(`/inboxes/${encodeURIComponent(inboxId)}/threads/${encodeURIComponent(message.thread_id)}`);
      const body = thread.messages?.map((item) => `${item.text ?? ''}\n${item.extracted_text ?? ''}`).join('\n') ?? '';
...
try {
  ...
  if (!recipientInboxId) {
    const listing = await agentMail('/inboxes?limit=100');
    const inbox = listing.inboxes?.find((candidate) => candidate.client_id === username);
  ...
  await page.getByRole('button', { name: 'Apple' }).waitFor();
  await page.getByRole('button', { name: 'New to RibbonDesk? Create an account' }).click();
  ...
  await page.getByRole('heading', { name: 'Confirm your email' }).waitFor({ timeout: 30_000 });

  const verificationUrl = await waitForSecurityLink(recipientInboxId, 'Verify your RibbonDesk email');
  ...
  const workspaceHeading = page.getByRole('heading', { name: 'Name your workspace' });
  try {
  ...
  await page.getByRole('button', { name: 'Passkey added' }).waitFor({ timeout: 30_000 });
  ...
  await page.getByText('If an account exists for that email, its reset link is queued for delivery.').waitFor({ timeout: 30_000 });
  const resetUrl = await waitForSecurityLink(recipientInboxId, 'Reset your RibbonDesk password');
  await page.goto(resetUrl, { waitUntil: 'networkidle' });
  await page.waitForURL((url) => url.origin === new URL(baseURL).origin, { timeout: 30_000 });
  await page.getByRole('heading', { name: 'Choose a new password' }).waitFor({ timeout: 30_000 });
  await page.getByLabel('New password').fill(nextPassword);
  await page.getByLabel('Confirm password').fill(nextPassword);
  await page.getByRole('button', { name: 'Update password' }).click();
  await page.getByText('Password updated. Sign in with your new password.').waitFor({ timeout: 30_000 });
  await page.getByLabel('Work email').fill(email);
  ...
  console.log(JSON.stringify({
    ...
    passkeySignIn: true,
    passwordReset: true,
    googleAndAppleControlsPresent: true,
  ...
  if (workspaceCreated) console.error('A controlled workspace may require cleanup after this failed run.');
