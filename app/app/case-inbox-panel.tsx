...
export function CaseInboxPanel({ locationId }: { locationId: Id<'locations'> }) {
  ...
  const receiveReplay = useMutation(api.inbox.receiveReplay);
  ...
  return (
    <section data-testid="case-inbox" className="mt-7 overflow-hidden rounded-[1.5rem] border bg-background" aria-labelledby="case-inbox-title">
      ...
      {!setup?.binding ? <div className="m-5 rounded-2xl border border-dashed bg-[var(--paper-strong)] p-7 text-center sm:m-6"><Mail className="mx-auto size-7 text-[var(--ribbon)]" /><p className="mt-3 font-semibold">Create this location’s dedicated case inbox</p><p className="mx-auto mt-2 max-w-xl text-xs leading-5 text-muted-foreground">Replay mode creates a safe synthetic thread. Live mode provisions AgentMail and activates its signed webhook path.</p><Button className="mt-4 bg-[var(--ribbon)] text-white hover:bg-[var(--ribbon-dark)]" onClick={() => void run('provision', () => provisionInbox({ locationId }), 'Case inbox requested. Its status updates here in realtime.')} disabled={!canApprove || pending === 'provision'}>{pending === 'provision' ? <LoaderCircle className="animate-spin" /> : <Plus />}Create case inbox</Button></div> : <>
        ...
        <div className="grid xl:grid-cols-[1.08fr_0.92fr]">
          <div className="border-b p-5 sm:p-6 xl:border-r xl:border-b-0">
            <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.17em] text-muted-foreground">Reactive thread</p><h3 className="mt-2 font-semibold">Messages</h3></div>{setup.binding.providerMode === 'replay' ? <Button size="sm" variant="outline" onClick={() => void run('replay', () => receiveReplay({ locationId }), 'A controlled inbound reply and review proposal were added.')} disabled={!canContribute || pending === 'replay'}><RotateCcw />Receive test reply</Button> : <Badge variant="outline"><MailCheck />Webhook live</Badge>}</div>
