...
import { canTransitionRequirement, isHttpsCitation } from './lib/domain';
    ...
    if (args.status) {
      return await ctx.db
        ...
        .withIndex('by_locationId_and_status', (query) =>
          query.eq('locationId', args.locationId).eq('status', args.status!),
        )
        .order('asc')
...
export const getGraph = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      requirements,
      edges,
      truncated: requirements.length === 250 || edges.length === 500,
    };
  },
});
...
export const createManual = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!isHttpsCitation(args.sourceUrl)) {
      throw new ConvexError({
        code: 'INVALID_SOURCE',
        message: 'Requirement sources must use HTTPS.',
      });
    }
    const source = new URL(args.sourceUrl);
    if (
      args.feeMinCents !== undefined &&
      args.feeMaxCents !== undefined &&
      args.feeMinCents > args.feeMaxCents
    ) {
      throw new ConvexError({
...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { identity, membership } = await requireLocation(
      ctx,
    ...
    if (
      ...
      if (args.status === 'confirmed')
        throw new ConvexError({
          ...
          message: 'An owner or admin must confirm requirements.',
        });
      throw new ConvexError({
        code: 'REVIEW_REQUIRED',
        message: 'Review the proposal before starting this requirement.',
      });
    }
    const now = Date.now();
    const patch = {
...
async function createsCycle(
  ...
  while (frontier.length > 0 && visited.size < 250) {
    ...
    const outgoing = await ctx.db
      .query('requirementEdges')
      .withIndex('by_fromRequirementId', (query) =>
        query.eq('fromRequirementId', current),
      )
      .take(100);
  ...
  if (frontier.length > 0) {
    throw new ConvexError({
      ...
      message: 'The dependency graph needs review before adding more edges.',
    });
  }
  return false;
}
...
export const addDependency = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !fromRequirement ||
      !toRequirement ||
      fromRequirement.locationId !== toRequirement.locationId
    ) {
      throw new ConvexError({
        code: 'NOT_FOUND',
        message: 'Both requirements must belong to the same location.',
      });
    }
    const { identity } = await requireLocation(
      ctx,
      fromRequirement.locationId,
      'contributor',
    );
    const existing = await ctx.db
      ...
      .withIndex('by_fromRequirementId_and_toRequirementId', (query) =>
        query
          .eq('fromRequirementId', args.fromRequirementId)
          .eq('toRequirementId', args.toRequirementId),
      )
      .unique();
