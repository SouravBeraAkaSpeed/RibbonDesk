...
function initialDueAt(requirement: Doc<'requirements'>, now: number) {
  if (requirement.deadline && requirement.deadline > now)
    return requirement.deadline;
  let dueAt =
    requirement.deadline ?? requirement.confirmedAt ?? requirement.capturedAt;
...
async function scheduleRenewalReminders(
  ...
  const now = Date.now();
  for (const cadenceDays of reminderCadences(dueAt, now)) {
    const scheduledFor = dueAt - cadenceDays * DAY;
    if (cadenceDays > 0 && scheduledFor > now) {
      await ctx.scheduler.runAt(
        scheduledFor,
        internal.operations.deliverRenewalReminder,
        { renewalCycleId, cadenceDays },
      );
    } else if (cadenceDays <= 1) {
  ...
  await ctx.db.patch(renewalCycleId, {
    remindersScheduledAt: now,
    updatedAt: now,
  });
}
...
async function createRenewalCycle(
  ...
  const existing = await ctx.db
    .query('renewalCycles')
    .withIndex('by_requirementId_and_dueAt', (index) =>
      index.eq('requirementId', input.requirementId).eq('dueAt', input.dueAt),
    )
    .unique();
...
async function notifyMembers(
  ...
  const memberships = await ctx.db
    ...
    .withIndex('by_organizationId', (index) =>
      index.eq('organizationId', input.organizationId),
    )
    .take(100);
...
export const getSetup = query({
  ...
  handler: async (ctx, args) => {
    ...
    const expiringDocuments = (
      await ctx.db
        .query('documents')
        .withIndex('by_locationId_and_expiresAt', (index) =>
          index.eq('locationId', args.locationId).gt('expiresAt', 0),
        )
        .order('asc')
        .take(100)
...
export const listNotifications = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query('notifications')
      .withIndex(
        'by_userTokenIdentifier_and_locationId_and_createdAt',
        (index) =>
          index
...
export const getPreferences = query({
  ...
  handler: async (ctx, args) => {
    ...
    return preferences
      ? {
          urgentEmail: preferences.urgentEmail,
...
export const updatePreferences = mutation({
  ...
  handler: async (ctx, args) => {
    const { identity, location } = await requireLocation(ctx, args.locationId);
    if (
      !Number.isInteger(args.digestHourLocal) ||
      args.digestHourLocal < 0 ||
      args.digestHourLocal > 23
    )
      throw new ConvexError({
        code: 'INVALID_HOUR',
        message: 'Digest hour must be between 0 and 23.',
      });
    ...
    const existing = await ctx.db
      ...
      .unique();
    const patch = {
      urgentEmail: args.urgentEmail,
      dailyDigest: args.dailyDigest,
...
export const markNotificationRead = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!notification || !notification.locationId)
      throw new ConvexError({
        code: 'NOT_FOUND',
        message: 'Notification not found.',
      });
    const { identity } = await requireLocation(ctx, notification.locationId);
    if (notification.userTokenIdentifier !== identity.tokenIdentifier)
      throw new ConvexError({
        code: 'FORBIDDEN',
...
export const createInspection = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.requirementId && args.applicationId)
      throw new ConvexError({
        code: 'ONE_LINK_ONLY',
        message:
          'Link an inspection to either a requirement or an application.',
      });
    if (args.requirementId) {
      ...
      if (!requirement || requirement.locationId !== args.locationId)
        throw new ConvexError({
          ...
          message: 'Requirement not found in this location.',
        });
    }
    if (args.applicationId) {
      const application = await ctx.db.get(args.applicationId);
      if (!application || application.locationId !== args.locationId)
        throw new ConvexError({
    ...
    if (!agency || !inspectionType)
      throw new ConvexError({
        code: 'INSPECTION_INCOMPLETE',
        message: 'Agency and inspection type are required.',
      });
    const now = Date.now();
...
export const recordInspectionOutcome = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (['passed', 'failed', 'completed'].includes(inspection.status))
      throw new ConvexError({
        code: 'ALREADY_RECORDED',
        message: 'This inspection already has a final outcome.',
      });
    const outcome = clean(args.outcome, 2_000);
    if (!outcome)
      throw new ConvexError({
        code: 'OUTCOME_REQUIRED',
        message: 'Record the inspection outcome.',
      });
    ...
    if (args.documentId) {
      ...
      if (
        !document ||
        document.locationId !== inspection.locationId ||
        document.status !== 'ready'
      )
        throw new ConvexError({
          code: 'INVALID_DOCUMENT',
          message:
            'Outcome evidence must be a checked document from this location.',
        });
    }
    const now = Date.now();
    ...
    if (args.status === 'failed' || args.status === 'reschedule_needed') {
      taskId = await ctx.db.insert('tasks', {
        ...
        title:
          args.status === 'failed'
            ? `Resolve ${inspection.inspectionType} findings`
            : `Reschedule ${inspection.inspectionType}`,
        description: outcome,
        status: 'not_started',
      ...
      if (inspection.requirementId)
        await ctx.db.patch(inspection.requirementId, {
...
export const trackRenewal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      ['proposed', 'conflicted', 'not_applicable'].includes(requirement.status)
    )
      throw new ConvexError({
        code: 'CONFIRM_FIRST',
        message:
          'Only a confirmed applicable requirement can become a renewal.',
      });
    if (!Number.isFinite(args.dueAt) || args.dueAt < Date.now() - DAY)
      throw new ConvexError({
        code: 'INVALID_DUE_DATE',
        message: 'Choose a current or future renewal date.',
      });
    const recurrenceRule =
      clean(args.recurrenceRule, 200) ??
      requirement.recurrenceRule ??
      'FREQ=YEARLY';
    const latest = await ctx.db
      .query('renewalCycles')
      .withIndex('by_requirementId_and_dueAt', (index) =>
        index.eq('requirementId', requirement._id),
      )
      .order('desc')
      .take(1);
...
export const startRenewal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!cycle)
      throw new ConvexError({
        code: 'NOT_FOUND',
        message: 'Renewal cycle not found.',
      });
    const { identity } = await requireLocation(
      ctx,
      cycle.locationId,
    ...
    if (!['upcoming', 'due', 'overdue'].includes(cycle.status))
      throw new ConvexError({
        code: 'INVALID_STATUS',
        message: 'This renewal cannot be started.',
      });
  ...
  handler: async (ctx, args) => {
    ...
    if (cycle.status === 'completed' || cycle.status === 'cancelled')
      throw new ConvexError({
        ...
        message: 'This renewal is already closed.',
      });
    const outcomeNotes = clean(args.outcomeNotes, 2_000);
    if (!outcomeNotes)
      throw new ConvexError({
        code: 'OUTCOME_REQUIRED',
        message: 'Record the renewal outcome.',
      });
    const now = Date.now();
    await ctx.db.patch(cycle._id, {
      status: 'completed',
    ...
    if (!requirement)
      throw new ConvexError({
        ...
        message: 'The linked requirement is unavailable.',
      });
  ...
  handler: async (ctx, args) => {
    ...
    for (const requirement of requirements) {
      if (
        !requirement.recurrenceRule ||
        !requirement.confirmedAt ||
        ['proposed', 'conflicted', 'not_applicable'].includes(
          requirement.status,
        )
      )
        continue;
      ...
      const existing = await ctx.db
        .query('renewalCycles')
        .withIndex('by_requirementId_and_dueAt', (index) =>
          index.eq('requirementId', requirement._id).eq('dueAt', dueAt),
        )
        .unique();
      if (existing) continue;
      await createRenewalCycle(ctx, {
        organizationId: requirement.organizationId,
        locationId: requirement.locationId,
        requirementId: requirement._id,
    ...
    await recordActivity(ctx, {
      ...
      action: 'location.operating_lifecycle_activated',
      entityType: 'location',
      entityId: location._id,
      after: { renewalCyclesCreated: created },
    });
    return created;
  },
});
...
export const deliverRenewalReminder = internalMutation({
  ...
  handler: async (ctx, args) => {
    const cycle = await ctx.db.get(args.renewalCycleId);
    if (!cycle || ['completed', 'cancelled'].includes(cycle.status))
      return null;
    const requirement = await ctx.db.get(cycle.requirementId);
    if (!requirement) return null;
    const now = Date.now();
    const overdue = cycle.dueAt < now;
    if (overdue && cycle.status !== 'overdue')
      await ctx.db.patch(cycle._id, { status: 'overdue', updatedAt: now });
    else if (args.cadenceDays <= 30 && cycle.status === 'upcoming')
      await ctx.db.patch(cycle._id, { status: 'due', updatedAt: now });
    if (
      args.cadenceDays <= 90 &&
      !['renewal_due', 'needs_attention'].includes(requirement.status)
    )
      await ctx.db.patch(requirement._id, {
        status: 'renewal_due',
        updatedAt: now,
...
export const scheduleDocumentReminders = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    for (const cadenceDays of reminderCadences(document.expiresAt, now)) {
      const scheduledFor = document.expiresAt - cadenceDays * DAY;
      if (cadenceDays > 0 && scheduledFor > now)
        await ctx.scheduler.runAt(
          scheduledFor,
          internal.operations.deliverDocumentReminder,
          { documentId: document._id, cadenceDays },
        );
      else if (cadenceDays <= 1)
        await ctx.scheduler.runAfter(
          0,
...
export const deliverDocumentReminder = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const timing = overdue
      ...
      : args.cadenceDays === 1
        ? 'expires tomorrow'
        : `expires in ${args.cadenceDays} days`;
    await notifyMembers(ctx, {
...
export const markOverdueRenewals = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const upcoming = await ctx.db
      .query('renewalCycles')
      .withIndex('by_status_and_dueAt', (index) =>
        index.eq('status', 'upcoming').lte('dueAt', now),
      )
      .take(100);
    const due = await ctx.db
      .query('renewalCycles')
      .withIndex('by_status_and_dueAt', (index) =>
        index.eq('status', 'due').lte('dueAt', now),
      )
      .take(100);
    for (const cycle of [...upcoming, ...due]) {
      ...
      await ctx.scheduler.runAfter(
        0,
        internal.operations.deliverRenewalReminder,
        { renewalCycleId: cycle._id, cadenceDays: 0 },
      );
    }
